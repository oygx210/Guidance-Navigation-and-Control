%% %% Project#1_Advanced_Control_Beam_Ball_System_Regulator_Phi_Full_Observer_Design
clc 
clear 

global m g J_Beam C_Yphi Ko_Yphi
...
%% Designing Observer Gain When System Observes X3=phi, Ackerman Method
...
while t < T
    ...
    X(:,k+1) = Xj;
    O1 = Beam_Ball_Phi_Full_Obser_Regual_Proj(t,Xhj,u,y);
    O2 = Beam_Ball_Phi_Full_Obser_Regual_Proj(t+dt/2,Xhj+O1*dt/2,u,y);
    O3 = Beam_Ball_Phi_Full_Obser_Regual_Proj(t+dt/2,Xhj+O2*dt/2,u,y);
    O4 = Beam_Ball_Phi_Full_Obser_Regual_Proj(t+dt,Xhj+O3*dt,u,y);   
...
title('Regulator With Full Order Observer "Phi" Controller Design Initial Conddition "0.2,0,10,0"')
xlabel('time(s)')
ylabel('x(m)')
