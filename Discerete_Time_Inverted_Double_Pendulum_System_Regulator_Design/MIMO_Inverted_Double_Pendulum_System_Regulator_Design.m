...
while t < T
    ...
    uj = ud(:,k);
    u(:,i) = uj;
    D1 = MIMO_DOuble_Pendulum_Regul_Proj(t,Xj,uj);
    D2 = MIMO_DOuble_Pendulum_Regul_Proj(t+dt/2,Xj+D1*dt/2,uj);
    D3 = MIMO_DOuble_Pendulum_Regul_Proj(t+dt/2,Xj+D2*dt/2,uj);
    D4 = MIMO_DOuble_Pendulum_Regul_Proj(t+dt,Xj+D3*dt,uj);
...
title('Regulator Controller Design and Sampling Time "0.05"')
xlabel('time(s)')
ylabel('x(m)')
