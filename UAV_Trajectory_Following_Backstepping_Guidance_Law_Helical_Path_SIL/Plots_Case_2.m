%% Plots_Guidance_and_Navigation_Project_Case_2
...
title('Position Tracking Errors (Case2)')
xlabel('Time (sec)')
ylabel('e_X (m)')
...
title('Speed(Vg),Elevatio(gamma),and Heading(zita) Tracking Errors(Case2)')
xlabel('Time (sec)')
ylabel('e_Vg (m/s)')
...
title('UAV and Target Flight Path 3D(Case2)')
legend('UAV','Target')
grid on
xlabel('X (m)')
ylabel('Y (m)')
