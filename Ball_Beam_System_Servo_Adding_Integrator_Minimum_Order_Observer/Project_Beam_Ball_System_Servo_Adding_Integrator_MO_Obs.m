%% Project#1_Advanced_Control_Beam_Ball_System_Servo_Adding_Integrator_Minimum_Order_Observer
clc 
clear 
...
while t < T
    ...
    D1 = Beam_Ball_Servo_Add_Int_MO_Obs_Proj(t,Xj,u,yr(k));
    D2 = Beam_Ball_Servo_Add_Int_MO_Obs_Proj(t+dt/2,Xj+D1*dt/2,u,yr(k));
    D3 = Beam_Ball_Servo_Add_Int_MO_Obs_Proj(t+dt/2,Xj+D2*dt/2,u,yr(k));
    D4 = Beam_Ball_Servo_Add_Int_MO_Obs_Proj(t+dt,Xj+D3*dt,u,yr(k));   
    Xj = Xj + (D1+2*D2+2*D3+D4)/6*dt;
    X(:,k+1) = Xj;
    O1 = Beam_Ball_Min_Obser_Servo_Add_Int_Proj(t,Ethj,u,y);
    O2 = Beam_Ball_Min_Obser_Servo_Add_Int_Proj(t+dt/2,Ethj+O1*dt/2,u,y);
    O3 = Beam_Ball_Min_Obser_Servo_Add_Int_Proj(t+dt/2,Ethj+O2*dt/2,u,y);
    O4 = Beam_Ball_Min_Obser_Servo_Add_Int_Proj(t+dt,Ethj+O3*dt,u,y);   
...
title('Integrator Servo Design with Minimum Order Observer Initial Conddition "0.2,0,10,0"')
xlabel('time(s)')
ylabel('x(m)')
