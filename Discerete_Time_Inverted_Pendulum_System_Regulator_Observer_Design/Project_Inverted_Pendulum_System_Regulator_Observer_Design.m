%% Project#2_Advanced_Control_Inverted_Pendulum_System_Regulator_Observer_Design
clc 
clear 

global M_Cart m g l Ko C
%% System Parameters
M_Cart = 2; %% Cart Mass
m = 0.5; %% Pendulum Mass
...
while t < T
    ...
    D1 = Pendulum_Regul_Obs_Proj(t,Xj,u(i));
    D2 = Pendulum_Regul_Obs_Proj(t+dt/2,Xj+D1*dt/2,u(i));
    D3 = Pendulum_Regul_Obs_Proj(t+dt/2,Xj+D2*dt/2,u(i));
    D4 = Pendulum_Regul_Obs_Proj(t+dt,Xj+D3*dt,u(i));   
...
%% Plots
figure;
subplot(5,1,1);plot(Time,X(1,:),Time(1:end-1),Xh(1,:),'g');
title('Regulator With Observer Design Initial Conddition "0,0,10,5"and Smapling Time of "0.01"')
xlabel('time(s)')
ylabel('x(m)')
